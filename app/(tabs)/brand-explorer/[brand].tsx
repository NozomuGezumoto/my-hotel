// ============================================
// Brand Detail - Hotels in this brand
// ============================================
...
export default function BrandDetailScreen() {
  ...
  const brandHotels = useMemo(
    () => hotels.filter((h) => getBrandForHotel(h) === brand),
    [hotels, brand]
  );

  const visitedCount = useMemo(
    () => brandHotels.filter((h) => visitedHotels.includes(h.id)).length,
    [brandHotels, visitedHotels]
  );
  const dreamCount = useMemo(
    () => brandHotels.filter((h) => wantToGoHotels.includes(h.id)).length,
    [brandHotels, wantToGoHotels]
  );
  const total = brandHotels.length;
  ...
  const titles = useMemo(
    () => [
      { key: 'master', label: t('titleMasterWithName', { name: brand }), condition: t('titleConditionMaster'), unlocked: hasMaster, icon: 'trophy' as const, tierColor: SUSHI_COLORS.titleMaster },
      { key: 'traveler', label: t('titleTravelerWithName', { name: brand }), condition: t('titleConditionTraveler'), unlocked: hasResident, icon: 'airplane' as const, tierColor: SUSHI_COLORS.titleTraveler },
      { key: 'explorer', label: t('titleExplorerWithName', { name: brand }), condition: t('titleConditionExplorer'), unlocked: hasExplorer, icon: 'compass' as const, tierColor: SUSHI_COLORS.titleExplorer },
    ],
    [t, brand, hasExplorer, hasResident, hasMaster]
  );
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Pressable style={styles.backButton} onPress={() => router.back()}>
        ...
        <Text style={styles.backLabel}>{t('brandExplorer')}</Text>
      </Pressable>

      <Text style={styles.title}>{brand}</Text>
      <Text style={styles.subtitle}>
        {t('visitedCount', { visited: String(visitedCount), total: String(brandHotels.length) })}
        {' · '}
      ...
      {/* ブランド称号 */}
      <Text style={styles.sectionTitle}>{t('brandTitles')}</Text>
